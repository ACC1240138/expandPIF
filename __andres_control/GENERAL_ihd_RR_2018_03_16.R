## This new relative risk function is supposed to fix all the silly little patches 
## that have been applied to this poor RR function over the years. 
## The method for dealing with bingers is the same as for ischaemic stroke now. 
...
#### Mortality ####
### male ###
IHDmaleMORT_1 = list(disease = "IHD_Mortality",
                     ...
                     lnRRFormer = log(1.25),
                     varLnRRFormer = 0.0427864176971434^2)
### female ###
IHDfemaleMORT_1 = list(disease = "IHD_Mortality",
                       RRCurrent = function(x, beta){ifelse(x>=0,
                                                            ifelse(x<30.3814,
                       ...
                       lnRRFormer = log(1.54),
                       varLnRRFormer = 0.140569399041845^2)


#### Mortality #### 
### male ###
IHDmaleMORT_2 = list(disease = "IHD_Mortality",
                     RRCurrent = function(x, beta){ifelse(x>=0,
                                                          ifelse(x<60,
                                                                 exp(beta[3] * (beta[1] * sqrt((x + 0.0099999997764826) / 100) +
                                                                                  beta[4] * ((x + 0.0099999997764826) / 100)^3)),
...
### female ###
IHDfemaleMORT_2 = list(disease = "IHD_Mortality",
                       RRCurrent = function(x, beta){ifelse(x>=0,
                                                            ifelse(x<30.3814,
                                                                   exp(beta[3] * (beta[1] * (x + 0.0099999997764826) / 100 +
                                                                                    beta[4] * (x + 0.0099999997764826) / 100 *
...
#### Mortality #### 
### male ###
IHDmaleMORT_3 = list(disease = "IHD_Mortality",
                     RRCurrent = function(x, beta){ifelse(x>=0,
                                                          ifelse(x<60,
                                                                 exp(beta[3] * (beta[1] * sqrt((x + 0.0099999997764826) / 100) +
                                                                                  beta[4] * ((x + 0.0099999997764826) / 100)^3)),
...
### female ###
IHDfemaleMORT_3 = list(disease = "IHD_Mortality",
                       RRCurrent = function(x, beta){ifelse(x>=0,
                                                            ifelse(x<30.3814,
                                                                   exp(beta[3] * (beta[1] * (x + 0.0099999997764826) / 100 +
                                                                                    beta[4] * (x + 0.0099999997764826) / 100 *
